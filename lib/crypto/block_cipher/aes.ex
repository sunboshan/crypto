defmodule Crypto.BlockCipher.AES do
  ...
  # Galois Field multiplication, tables from http://www.adamberent.com/documents/AESbyExample.pdf
  @e {
  ...
  defp substitute_1(bin) do
    for <<i::4, j::4 <- bin>>, into: <<>>, do: <<elem(elem(@s_1, i), j)>>
  end
  ...
  defp mix_column_1(bin) do
    for <<a, b, c, d <- bin>>, <<i, j, k, l <- @matrix_1>>, into: <<>> do
      <<gf(i, a) ^^^ gf(j, b) ^^^ gf(k, c) ^^^ gf(l, d)>>
    end
  end
